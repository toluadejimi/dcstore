                    ...
                    <table class="table table--light style--two">
                        ...
                        @forelse($deposits as $deposit)
                            ...
                            @if ($deposit instanceof \App\Models\Deposit)
                                <tr>
                                    <td>
                                        <span class="fw-bold"> <a href="{{ appendQuery('method',@$deposit->gateway->alias) }}">{{ __(@$deposit->gateway->name) }} (Direct Deposit)</a> </span>
                                        <br>
                                    ...
                                    <td>
                                        <a href="{{ route('admin.deposit.details', $deposit->id) }}?source=deposit"
                                        class="btn btn-sm btn-outline--primary ms-1">
                                            <i class="la la-desktop"></i> @lang('Details')
                                        </a>
                                    </td>
                                </tr>
                            @elseif ($deposit instanceof \App\Models\WalletHistory)
                                <tr>
                                    <td>
                                        <span class="fw-bold"> <a href="{{ appendQuery('method',@$deposit->gateway->alias) }}">{{ __(@$deposit->gateway->name) }}</a> (Wallet Credit) </span>
                                        <br>
                                        <small> {{ $deposit->trx }} </small>
                                    </td>
                                    ...
                                    <td>
                                        <span class="fw-bold">{{ $deposit->wallet->user->fullname }}</span>
                                        <br>
                                        <span class="small">
                                        <a href="{{ appendQuery('search',@$deposit->wallet->user->username) }}"><span>@</span>{{ $deposit->wallet->user->username }}</a>
                                        </span>
                                    </td>
                                    <td>
                                    {{ __($general->cur_sym) }}{{ showAmount($deposit->amount ) }} + <span class="text-danger" title="@lang('charge')">{{ showAmount($deposit->charge)}} </span>
                                    ...
                                    <td>
                                        <a href="{{ route('admin.deposit.details', $deposit->id) }}?source=wallet"
                                        class="btn btn-sm btn-outline--primary ms-1">
                                            <i class="la la-desktop"></i> @lang('Details')
                                        </a>
                                    </td>
